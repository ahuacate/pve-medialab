#!/usr/bin/env bash
# ----------------------------------------------------------------------------------
# Filename:     pve_medialab_ct_whisparr_installer.sh
# Description:  This script is for creating a Proxmox Whisparr CT
# ----------------------------------------------------------------------------------
...
pct exec $CTID -- bash -c "export REPO_PKG_NAME=${REPO_PKG_NAME} && /tmp/servarr_ct_sw.sh"
...
A ${REPO_PKG_NAME^} settings preset file is included. You need Go to ${REPO_PKG_NAME,,^} WebGUI 'System' > 'Backup' and restore the backup filename:
