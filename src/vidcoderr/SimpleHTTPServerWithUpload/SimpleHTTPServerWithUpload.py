...
class SimpleHTTPRequestHandler(http.server.BaseHTTPRequestHandler):
    ...
    # Basic version with no progress bar
    def do_POST(self):
        ...
        f.write(b'* {font-family: "San Francisco", "Helvetica Neue", Helvetica; font-size: 12px; }\n')
        f.write(b'a { text-decoration: none; }\n')
        f.write(b'</style>\n')
        f.write(b"<body>\n<h2>Upload Result Page</h2>\n")
        f.write(b"<hr style='height: 1px; width: 100%; border: none; background-color: black;'>\n")
        if r:
        ...
        f.write(b"<small>Powered By: bones7456<br>Check new version ")
    ...
    def list_directory(self, path):
        ...
        f.write(b'th, td {\n  padding:0px 10px;\n}\n')
        f.write(b'</style>\n')
        # Read the contents of queue.txt and write them to a table
        queue_path = '/usr/local/bin/vidcoderr/queue.txt'
        if os.path.exists(queue_path):
            with open(queue_path) as f_queue:
                queue_contents = f_queue.read().splitlines()
            if queue_contents:
                f.write(b"<div style='background-color: #f0f0f0; padding: 10px; margin: 0px;'>\n")
                f.write(("<h2>Video files in Vidcoderr processing queue</h2>\n").encode(enc))
                f.write(("<meta http-equiv='refresh' content='60'>\n").encode(enc)) # Refresh the page every 60 seconds
                f.write(("<p>The Vidcoderr encoding queue currently contains %d files. As video encoding tends to be a slow process, please exercise patience. The queue list will be refreshed every 60 seconds. You can confirm that your new video is ready when it is no longer visible in the queue list.</p>\n" % len(queue_contents)).encode(enc))
                f.write(b"<table><tr><th>Label</th><th style='text-align:left'>Filename</th></tr>\n")
                for line in queue_contents:
                    columns = line.split(';')
                    filename = columns[0]
                    label = columns[3]
                    f.write(b"<tr>")
                    f.write(("<td>{}</td><td style='text-align:left'>{}</td>".format(label, filename)).encode(enc))
                    f.write(b"</tr>\n")
                f.write(b"</table>\n")
                f.write(b"</div>\n")
            else:
                f.write(b"<div style='background-color: #f0f0f0; padding: 10px; margin: 0px;'>\n")
                f.write(("<h2>Video files in Vidcoderr processing queue</h2>\n").encode(enc))
                f.write(("<p>.... Waiting. Nothing to encode.</p>\n").encode(enc))
                f.write(b"</div>\n")
        f.write(("<body>\n<h2>Directory listing for %s</h2>\n" % os.path.abspath(path)).encode(enc))
        f.write(("<p>Shown below are two upload folders. The 'in_homevideo' folder is dedicated to home video media only and utilizes preset profiles to achieve the best visual quality. Conversely, the 'in_unsorted' folder is designed for other media types, like movies and TV series. To upload your video for encoding, choose your preferred folder and use the 'browse' button to select the video file. Then click 'upload' to send the file to Vidcoderr. The processing occurs either every 6 hours (default setting) or sooner if InotifyWait functions effectively within your network. The encoded video output file will be saved in one of their designated folders:</p>\n").encode(enc))
        f.write("<table>\n".encode(enc))
        f.write("<tr>\n".encode(enc))
        f.write("<td>-- 'in_unsorted'</td>\n".encode(enc))
        f.write("<td>&nbsp;&rarr;&nbsp;</td>\n".encode(enc))
        f.write("<td>'/mnt/public/autoadd/vidcoderr/out_unsorted' folder. Right-click on your new encoded video and select \"Save as\" to save to your computer.</td>\n".encode(enc))
        f.write("</tr>\n".encode(enc))
        f.write("<tr>\n".encode(enc))
        f.write("<td>-- 'in_homevideo'</td>\n".encode(enc))
        f.write("<td>&nbsp;&rarr;&nbsp;</td>\n".encode(enc))
        f.write("<td>'.../video/homevideo' your main home video folder</td>\n".encode(enc))
        f.write("</tr>\n".encode(enc))
        f.write("</table>\n".encode(enc))
        f.write(("<p>Please make sure to upload only video files. After uploading, within 6hrs your video file will appear in the Vidcoderr processing queue list. When Vidcoderr has finished encoding your new video, the name of the video will no longer appear in the queue list.</p>\n").encode(enc))
        f.write(b"<hr style='height: 1px; width: 100%; border: none; background-color: black;'>\n")
        ...
        for name in list:
                if not name.startswith('.'):
                    ...
                    if name.endswith(('.bmp','.gif','.jpg','.png')):
                        dirimage = name
                    if name.endswith(('.avi','.mpg','.mp4','.mkv')):
        ...
        f.write(b"</table><hr style='height: 1px; width: 100%; border: none; background-color: black;'>\n</body>\n</html>\n")
        length = f.tell()
        f.seek(0)
        self.send_response(200)
        self.send_header("Content-type", "text/html")
