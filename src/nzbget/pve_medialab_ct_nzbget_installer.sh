...
# Nzbget SW
pct push $CTID ${SRC_DIR}/nzbget/nzbget_sw.sh /tmp/nzbget_sw.sh -perms 755
pct exec $CTID -- bash -c "export REPO_PKG_NAME=${REPO_PKG_NAME} APP_USERNAME=${APP_USERNAME} APP_GRPNAME=${APP_GRPNAME} && /tmp/nzbget_sw.sh"
...
if [ -f ${SRC_DIR}/${REPO_PKG_NAME,,}/config/${REPO_PKG_NAME,,}_backup_*_0000.00.00_00.00.00.zip ]; then
  ...
  pct exec $CTID -- runuser ${APP_USERNAME} -c "mkdir -p /var/lib/nzbget/Backups/manual"
  pct push $CTID ${SRC_DIR}/${REPO_PKG_NAME,,}/config/${BACKUP_FILE} /var/lib/nzbget/Backups/manual/${BACKUP_FILE}
fi
