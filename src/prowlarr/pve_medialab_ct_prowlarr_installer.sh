...
if [ -f ${SRC_DIR}/${REPO_PKG_NAME,,}/config/${REPO_PKG_NAME,,}_backup_*_0000.00.00_00.00.00.zip ]; then
  ...
  BACKUP_PRESET_FILE=$(find ${SRC_DIR}/${REPO_PKG_NAME,,}/config/ -name *_0000.00.00_00.00.00.zip -type f -exec basename {} 2> /dev/null \;)
  pct exec $CTID -- mkdir -p /var/lib/${REPO_PKG_NAME,,}/Backups/manual
  pct push $CTID ${SRC_DIR}/${REPO_PKG_NAME,,}/config/${BACKUP_PRESET_FILE} /var/lib/${REPO_PKG_NAME,,}/Backups/manual/${BACKUP_PRESET_FILE}
  # Set Backup file status
  BACKUP_PRESET_FILE_STATUS=1
fi
...
# Backup preset file
if [ "${BACKUP_PRESET_FILE_STATUS}" == '1' ]; then
  display_msg2=( "Local: ${BACKUP_PRESET_FILE}" )
else
  display_msg2=( "Local: 'preset file not available'" )
fi
# Backup preset description
display_msg3=( "--  Media Management (sources)" \
"--  General ${REPO_PKG_NAME^} setup" \
"--  Set backup destination: /mnt/backup/${REPO_PKG_NAME^} (preset to use NAS)" )
# Check CT domain
...
If our ${REPO_PKG_NAME^} settings preset file is included go to ${REPO_PKG_NAME,,^} WebGUI 'System' > 'Backup' and restore the backup filename:
