#!/usr/bin/env bash
# ----------------------------------------------------------------------------------
# Filename:     kodirsync_clientapp_node_rsync_main.sh
# Description:  Rsync transfer cmd script for '$kodirsync_storage' files
...
then
  ...
  function rsync_with_retries {
    ...
    while [ $retries -lt $max_retries ]; do
      ...
      --timeout=60 \
      --human-readable \
      --partial-dir=$node_dst_dir/rsync_tmp \
      --delete \
      --delete-before \
      --exclude '*.partial~' \
      --log-file=$logfile \
      --files-from=$work_dir/rsync_storage_list.txt \
      --relative \
      --no-owner \
      --modify-window=1 \
      --size-only \
      "$local_dst_dir" $node_user@$lan_address:"$node_dst_dir"
