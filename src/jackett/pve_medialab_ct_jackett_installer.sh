...
# Description:  This script is for creating a Proxmox Jackett CT
# ----------------------------------------------------------------------------------
...
pct push $CTID $SRC_DIR/jackett/jackett_sw.sh /tmp/jackett_sw.sh -perms 755
pct exec $CTID -- bash -c "export REPO_PKG_NAME=$REPO_PKG_NAME APP_USERNAME=$APP_USERNAME APP_GRPNAME=$APP_GRPNAME && /tmp/jackett_sw.sh"

#---- Configure SW
pct push $CTID $COMMON_DIR/bash/src/basic_bash_utility.sh /tmp/basic_bash_utility.sh -perms 755
pct push $CTID $SRC_DIR/jackett/config/jackett_config.sh /tmp/jackett_config.sh -perms 755
pct exec $CTID -- bash -c "export REPO_PKG_NAME=$REPO_PKG_NAME APP_USERNAME=$APP_USERNAME APP_GRPNAME=$APP_GRPNAME && /tmp/jackett_config.sh"
...
The credentials for third party application connectivity are shown in the ${REPO_PKG_NAME^} application Web-interface.
