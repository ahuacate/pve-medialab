#!/usr/bin/env bash
# ----------------------------------------------------------------------------------
# Filename:     pve_medialab_ct_notifiarr_installer.sh
# Description:  This script is for creating a Proxmox Notifiarr CT
# ----------------------------------------------------------------------------------
...
# Notifiarr SW (shared script)
pct push $CTID $COMMON/bash/src/basic_bash_utility.sh /tmp/basic_bash_utility.sh -perms 755
pct push $CTID $SRC_DIR/notifiarr/notifiarr_sw.sh /tmp/notifiarr_sw.sh -perms 755
pct exec $CTID -- bash -c "export REPO_PKG_NAME=$REPO_PKG_NAME APP_USERNAME=$APP_USERNAME APP_GRPNAME=$APP_GRPNAME && /tmp/notifiarr_sw.sh"
...
msg_box "${REPO_PKG_NAME^} installation was a success. Your first step is to open the ${REPO_PKG_NAME^} conf file, set your Notifiarr API Key and restart the client. The first start-up may take a few seconds so be patient. Web-interface is available on:
